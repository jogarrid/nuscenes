...
class NuScenes:
    ...
    def my_render_scene_on_map(self, scene_token: str) -> None:
        self.explorer.my_render_scene_on_map(scene_token)
...
class NuScenesExplorer:
    ...
    def my_render_scene_on_map(self, scene_token: str) -> None:
        ...
        # For the purpose of this demo, we don-t subsample
        demo_ss_factor = 1.0

        # Now draw the map mask
        mask = Image.fromarray(map_mask.mask)
        mask_downsampled = mask.resize((int(mask.size[0]/demo_ss_factor), int(mask.size[1]/demo_ss_factor)),
                                resample=Image.NEAREST)

        axes.imshow(mask_downsampled)
        pos_x = []
        pos_y = []
        trajectories = []
        for sample_token in sample_tokens:
            ...
            # Plot
            pos_x.append(map_pose[0] / demo_ss_factor)
            pos_y.append(map_pose[1] / demo_ss_factor)
            ...
            for ann_token in annotations:
                ...
                instance = self.nusc.get('instance', ann['instance_token'])   
                 
                #If this is the first annotation for this instance, we save its trajectory (else, it already has been done)
                traj = []
                if(instance['first_annotation_token'] == ann['token']):
                    ann_record = ann
                    traj.append(ann_record['translation']+ [1])
                    while not ann_record['next'] == "":
                        ann_record = self.nusc.get('sample_annotation', ann_record['next'])
                        traj.append(ann_record['translation'] + [1])
        ...
        #Plot trajectory of all instances
        colors = ['g', 'r', 'c', 'm', 'y', 'k', 'w'] #blue being reserved for the ego vehicle
        color_index = 0;
        for t in trajectories:
            pos_annx = []
            pos_anny = []
            for pos_ann in t:   #posann, position of an annotation
               map_posann = np.dot(map_mask.transform_matrix, pos_ann)
               pos_annx.append(map_posann[0])
               pos_anny.append(map_posann[1])
            axes.plot(pos_annx,pos_anny, colors[color_index%len(colors)])
            color_index = color_index + 1
            print(min(pos_annx))
            xmin = min(xmin, min(pos_annx))
            xmax = max(xmax, max(pos_annx))
            ymin = min(ymin, min(pos_anny))
            ymax = max(ymax, max(pos_anny))


        #plot ego vehicle trajectory
        axes.plot(pos_x,pos_y, 'b.')
